  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
      ...
      <div className="flex items-center gap-4 mb-5">
        <div className="w-14 h-14 bg-gradient-to-br from-sky-500 to-sky-600 rounded-full flex items-center justify-center text-white text-xl font-bold shadow-md">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-sky-50 border border-sky-100 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-2">
            <div className="p-1.5 bg-sky-200 rounded-md">
              <Clock className="h-4 w-4 text-sky-600" />
            </div>
            <span className="text-xs font-medium text-sky-700">달성 금액</span>
          </div>
          <p className="text-lg font-bold text-sky-600">
            {completedSales?.toLocaleString()}
            <span className="text-sm font-normal text-sky-500 ml-1">원</span>
          </p>
        </div>
      </div>
      ...
      {user?.target && (
        <div className="mt-4">
          <div className="flex justify-between items-center mb-1.5">
            ...
            <span className={`text-xs font-bold ${targetRate >= 100 ? "text-emerald-600" : "text-sky-600"}`}>
              {targetRate}%
            </span>
          </div>
          <div className="h-2 bg-slate-200 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all ${
                targetRate >= 100 ? "bg-emerald-500" : "bg-sky-500"
              }`}
